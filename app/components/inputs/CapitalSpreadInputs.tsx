        ...
        {fields.map((item, index) => (
          <div key={item.id} className="border border-gray-200 rounded-lg p-3 space-y-2">
                ...
                <Controller
                  ...
                  render={({ field, fieldState: { error } }) => (
                    <NumberField
                      {...field}
                      label=""
                      prefix={capitalSpread[index].isPercentage ? "" : "$"}
                      suffix={capitalSpread[index].isPercentage ? "%" : ""}
                      min={0}
                      error={error?.message}
                    />
                  )}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setValue(`capitalSpread.${index}.isPercentage`, !capitalSpread[index].isPercentage)}
                className={`px-2 py-1.5 text-xs rounded-md border ${
                  capitalSpread[index].isPercentage
